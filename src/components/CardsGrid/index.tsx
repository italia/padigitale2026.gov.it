...
import { fetchAnnouncements } from "@/src/components/CardsGrid/actions";
...
export function CardsGrid({
  ...
  useEffect(() => {
    if (__typename === "CardsGridAnnouncementRecord") {
      startTransition(async () => {
        ...
        setFetchedAnnouncements(updatedViews);
      });
    }
  }, [props, __typename]);
    ...
    if (fetchedAnnouncements) {
      ...
      announcements = fetchedAnnouncements.map((announcement: Avviso) => {
        ...
        return {
          ...
          dataDiScadenza: new Date(announcement.endDate).toLocaleDateString(
            "it-IT",
            {
              day: "numeric",
              month: "long",
  ...
  if (__typename === "CardsGridNewsRecord") {
    ...
    cardLayout =
      typeof props.cardLayout !== "undefined"
        ? props.cardLayout
        : newsCardLayoutEnum.clean;

    newsSelection =
      typeof props.newsSelection !== "undefined" ? props.newsSelection : null;
    ...
    >
      ...
      >
        ...
        >
          ...
          {(news &&
            news.length &&
            (!newsSelection || newsSelection !== "paginated") &&
            (() => {
              if (columns === 1) {
                colClasses = "col-12";
              } else if (columns === 2) {
              ...
              return (
                <>
                  ...
                  >
                    <Row role={"list"} className={"h-100"}>
                      {news.map((newsRecord: NewsRecord, idx) => {
                        ...
                        if (shouldHide && news.length >= itemsPerPage)
                          return null;
                  ...
                  {news.length > itemsPerPage && (
                    ...
                    >
                      <Pager
                        aria-label="Naviga tra le pagine di questa lista di notizie"
                        role="navigation"
                      >
