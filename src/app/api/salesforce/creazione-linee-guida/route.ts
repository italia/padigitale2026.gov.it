  ...
  try {
    ...
    if (entity_content) {
      ...
      record = {
        ...
        Misura__c: entity_content.misura.filter(m => m.idSalesforce).map(m => m.idSalesforce).join(';'),
        Pacchetto__c: entity_content.misura.filter(m => m.pacchetto).map(m => m.pacchetto).join(';'),
